...
public Transform tPlayer;	//the main character transform
public Transform tPlayerRotation;	//Player child transform to rotate it in game
public Animation aPlayer;				//character animation

public Transform tPlayerSidesCollider;	//sides collider transform (detects stumble)
public Transform tFrontCollider;			//front collider transfrom (detects collisions)
public Vector3 v3BNCDefaultScale;
public Vector3 v3BFCDefaultScale;

//Variables
public float fCurrentWalkSpeed;

public float tCurrentAngle = 0.0f;	//current rotation along Y axis
public float fJumpForwardFactor = 0.0f;	//movement speed increase on jump
public float fCurrentUpwardVelocity = 0.0f;	//speed during the duration of jump
public float fCurrentHeight = 0.0f;
public float fContactPointY = 0.0f;	//y-axis location of the path

//player state during gameplay
public bool  bInAir = false;
public bool  bJumpFlag = false;
public bool  bInJump = false;
public bool  bInDuck = false;				//true if the character is sliding
public bool  bDiveFlag = false;			//force character to dive during jump
public bool  bExecuteLand = false;
public bool  bInStrafe = false;

public float fForwardAccleration = 0.0f;
public Transform tBlobShadowPlane;	//the shadow under the player
public Vector3 CurrentDirection;//set player rotation according to path

//script references
public PatchesRandomizer hPatchesRandomizer;
public CheckPointsMain hCheckPointsMain;
public InGameScript hInGameScript;
public PitsMainController hPitsMainController;
public SoundManager hSoundManager;
public CameraController hCameraController;
public PowerupsMainController hPowerupScript;
public EnemyController hEnemyController;
public MenuScript hMenuScript;
public PlayerFrontColliderScript hPlayerFrontColliderScript;
public PlayerSidesColliderScript hPlayerSidesColliderScript;

public RaycastHit hitInfo;	//whats under the player character
public bool  bGroundhit = false;	//is that an object under the player character
public float fHorizontalDistance = 0.0f;	//calculate player's horizontal distance on path

public float fCurrentForwardSpeed = 0.5f;	//sets movement based on spline
public float fCurrentDistance = 0.0f;//distance between the start and current position during the run
public float fCurrentMileage = 0.0f;//used to calculate the score based on distance covered

//detect if there is a terrain_lyr under the player
public float fPitFallLerpValue = 0.0f;
public float fPitFallForwardSpeed = 0.0f;
public float fPitPositionX = 0.0f;	//check the position of the pit in x-axis
public int iDeathAnimStartTime = 0;
public int iDeathAnimEndTime = 3;	//duration wait for death scene

public bool  JumpAnimationFirstTime = true;	//play death animation once
public Camera HUDCamera;

public Transform tPauseButton;
public Transform tHUDGroup;
public SwipeControls swipeLogic;
public int iLanePosition;						//current lane number -- -1, 0 or 1
public int iLastLanePosition; 					//stores the previous lane on lane change
public bool  bMouseReleased = true;
public bool  bControlsEnabled = true;

//action queue
    public SwipeControls.SwipeDirection directionQueue;
public bool  bDirectionQueueFlag = false;
...
public float fJumpPush = 185;			//force with which player pushes the ground on jump
public int getAccleration (){ return 500; }	//accleration and deceleration on jump

//the initial distance of the player character at launch
//from the start of the path
public float fCurrentDistanceOnPath = 0.0f;

//Is used to switch between gyro and swipe controls
public bool  swipeControlsEnabled = true;
